  ...
  const onSubmit = async (data: FormValues) => {
    if (tab === "signup") {
      ...
      if (success) {
        ...
        toast.error(`Failed to create ${selectedRole === "ngo" ? "organization" : "user"} account. Please check your information and try again.`);
      }
    } else {
      const success = await login(data.email, data.password, data.role as any);
      if (success) {
        toast.success(`Welcome back! Successfully logged in as ${selectedRole === "ngo" ? "organization" : selectedRole}.`);
        router.push("/");
      } else {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 space-y-6 border border-gray-200 dark:border-gray-700">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* Full Name / Organization Name Field for signup only */}
            {tab === "signup" && (
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  {selectedRole === "ngo" ? "Organization Name" : "Full Name"} <span className="text-red-500">*</span>
                </label>
                <div className="relative">
                  <User className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-400" />
                  <input
                    type="text"
                    placeholder={selectedRole === "ngo" ? "Enter your organization name" : "Enter your full name"}
